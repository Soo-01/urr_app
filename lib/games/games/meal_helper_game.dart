...
class MealHelperFlameGame extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    if (config.cognitiveLevel.showTimer) {
      ...
      _timerText =
          _HudText(text: '', pos: Vector2.zero(), anchor: Anchor.topLeft);
    }
  ...
  void _applyDifficulty() {
    ...
    _requiredHoldTime =
        const {2: 0.8, 3: 1.0, 4: 1.2, 5: 1.5, 6: 2.0}[stage] ?? 1.2;
    _targetRadius =
        const {2: 0.25, 3: 0.20, 4: 0.15, 5: 0.12, 6: 0.08}[stage] ?? 0.15;
  ...
  void _generateNewFood() {
    ...
    _targetPosition =
        margin + 0.5 * romR + _rng.nextDouble() * 0.4 * romR; // 중간~높은 위치 (팔 뻗기)
    _holdTime = 0;
    _inTarget = false;
  ...
  void _onPhase1Complete() {
    ...
    add(_FloatingLabel(
        pos: Vector2(size.x * 0.5, size.y * 0.35),
        text: '✅ 음식 잡기 성공!',
        color: Colors.greenAccent));

    // 모터 전환: 어깨 → 팔꿈치
    motor?.safeStop();
    motor?.selectJoint('rElbow');
  ...
  void _onPhase2Complete() {
    ...
    add(_FloatingLabel(
        pos: Vector2(size.x * 0.5, size.y * 0.3),
        text: '$foodName 맛있다!',
        color: _foodColors[_currentFood]));
    ...
    add(ParticleSystemComponent(
        ...
        particle: fp.Particle.generate(
            ...
            generator: (i) => fp.AcceleratedParticle(
                speed: Vector2(
                    _rng.nextDouble() * 100 - 50, -_rng.nextDouble() * 80),
                acceleration: Vector2(0, 200),
                child: fp.CircleParticle(
                    radius: 2 + _rng.nextDouble() * 3,
                    paint: Paint()
                      ..color =
                          _foodColors[_currentFood].withValues(alpha: 0.7))))));
  }
  ...
  void endGame() {
    ...
    onGameEnd(GameResult(
      gameId: 'meal_helper',
      score: score,
      maxPossibleScore: score + missed * 30,
      accuracy: (eaten + missed) > 0
          ? eaten / (eaten + missed)
          : (eaten > 0 ? 1.0 : 0.0),
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawOval(
        Rect.fromCenter(center: Offset(cx, plateY), width: 180, height: 50),
    ...
    canvas.drawArc(
        Rect.fromCenter(center: Offset(cx, mouthY), width: 50, height: 30),
        0.2,
        pi - 0.4,
        false,
        Paint()
    ...
    if (_phase == 2) {
      // 입 열기
      canvas.drawArc(
          Rect.fromCenter(center: Offset(cx, mouthY), width: 50, height: 40),
          0.3,
          pi - 0.6,
          false,
          Paint()
    ...
    final zoneColor = _inTarget
        ? Colors.greenAccent.withValues(alpha: 0.2)
        : Colors.amberAccent.withValues(alpha: 0.1);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromCenter(
                center: Offset(cx + 100, targetY), width: 60, height: zoneH),
    ...
    if (progress > 0 && !_transitioning) {
      ...
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTWH(cx - 60, size.y * 0.85, barW, 10),
              const Radius.circular(5)),
          Paint()..color = Color.lerp(Colors.orange, Colors.green, progress)!);
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTWH(cx - 60, size.y * 0.85, 120, 10),
              const Radius.circular(5)),
          Paint()
    ...
    TextPaint(
        ...
        .render(canvas, guideText, Vector2(cx, size.y * 0.93),
    ...
    for (int i = 0; i < min(eaten, 10); i++) {
      ...
      canvas.drawCircle(
          Offset(fx, size.y * 0.42),
          8,
          Paint()
            ..color =
                _foodColors[i % _foodColors.length].withValues(alpha: 0.6));
    }
  }
  ...
  void _drawSpoon(Canvas canvas, Offset pos, bool hasFood) {
    ...
    if (hasFood) {
      canvas.drawOval(
          Rect.fromCenter(center: const Offset(0, -1), width: 12, height: 7),
...
class _FloatingLabel extends PositionComponent {
  ...
  double _life = 0;
  _FloatingLabel(
      {required Vector2 pos, required this.text, required this.color})
...
class _HudText extends PositionComponent {
  ...
  _HudText(
      ...
      : _text = text,
        _color = color,
        super(position: pos, anchor: anchor, priority: 100);
...
class _KitchenBackground extends PositionComponent {
  final Vector2 _gs;
  _KitchenBackground({required Vector2 gameSize})
      : _gs = gameSize,
        super(priority: -10);
  @override
  void render(Canvas canvas) {
...
class _MealHelperGameState extends State<MealHelperGame> {
  ...
  void initState() {
    ...
    _game = MealHelperFlameGame(
        inputStream: stream,
        config: widget.config,
        motor: _isSim ? null : _motor,
        onGameEnd: (r) {
          _motor.safeStop();
          _motor.dispose();
          if (mounted)
    ...
    if (!_isSim) {
      _motor.selectJoint('rShoulderEF'); // 1단계: 어깨부터 시작
